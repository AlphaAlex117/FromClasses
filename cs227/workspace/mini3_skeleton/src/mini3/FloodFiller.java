...
public class FloodFiller
{
  ...
  {
	  if (row >= grid.length || col >= grid[0].length || row < 0 || col < 0 || grid[row][col].isBoundary())
	  {
		  return;
	  }
	  else if (grid[row][col].getStatus() == Status.SEEN || grid[row][col].getStatus() == Status.EXPLORE_UP || grid[row][col].getStatus() == Status.EXPLORE_DOWN || grid[row][col].getStatus() == Status.EXPLORE_LEFT || grid[row][col].getStatus() == Status.EXPLORE_RIGHT || grid[row][col].getStatus() == Status.DONE)
	  {
		  return;
	  }
	  else
