...
public class SplayTree<E extends Comparable<? super E>> extends AbstractSet<E>
{
	...
	{
		// Assigns as root the node returned by the method.
	...
	{	
		...
		else
		{
			...
			{
				cloneTreeRec(subTree.left);
			}
			
			if (subTree.right != null)
			{
				cloneTreeRec(subTree.right);
			}
		}
	...
	{
		// If root is null, then create a new node for the root.
		if (root == null)
		{
			root = new Node(data);
			size++;
		...
		while (true)
		{
			// Compares the current node's data vs the input data.
			...
			// If input data is less, then place it on the left.
			else if (comp > 0)
			{
			...
			// If input data is greater, then place it on the right.
			else
			{
	...
	public boolean contains(E data)
	{
		Node foundNode = findEntry(data);
		splay(foundNode);
		return foundNode.data.compareTo(data) == 0; 
	}
	...
	public Node findEntry(E data)
	{
		 ...
		 *  Otherwise:
		 *  
		 *  While current is not null, keep searching until you find the element. 
		 *  If such element does not exist, return the last node on the path..
		 */
		while (current != null)
		{
			int comp = current.data.compareTo(data);
		...
		// Return last node on the search path.
		return previousNode; 
	}
	...
	public Node join(Node root1, Node root2)
	{
		...
		if (root1.data.compareTo(current.data) < 0)
		{
			return root2;
		}
		else
	...
	protected void splay(Node current)
	{
		while (current != root)
		{
			...
			Node grandParentNode = current.parent.parent;
			
			if (grandParentNode == null)
			{
				zig(current);
			}
			else if ((grandParentNode.data.compareTo(parentNode.data) > 0 && parentNode.data.compareTo(current.data) > 0) || (grandParentNode.data.compareTo(parentNode.data) < 0 && parentNode.data.compareTo(current.data) < 0))
			{
				zigZig(current);
			}
			else if ((grandParentNode.data.compareTo(parentNode.data) > 0 && parentNode.data.compareTo(current.data) < 0) || (grandParentNode.data.compareTo(parentNode.data) < 0 && parentNode.data.compareTo(current.data) > 0))
	...
	protected void zigZig(Node current)
	{
		...
		if (grandParentNode.data.compareTo(parentNode.data) > 0 && parentNode.data.compareTo(current.data) > 0)
		{
			rightRotate(current.parent);
			rightRotate(current);
		}
		else if (grandParentNode.data.compareTo(parentNode.data) < 0 && parentNode.data.compareTo(current.data) < 0)
		{
	...
	{
		// This is for linking the subtrees
		current.parent.right = current.left;
		current.left = current.parent;
		link(current.parent.parent, current);
		...
		{
			link(current.left,current.left.right);
		}
	}
	...
	public void rightRotate(Node current)
	{
		...
		{
			link(current.right,current.right.left);
		}
	}	
	...
	private String toStringRec(Node n, int depth)
	{
		...
		// Stores the current node in the string.
		printedTree = spaces + n.data + "\n";
		...
		else
		{
			if (n.left != null)
			{
				left = toStringRec(n.left, depth + 1);
			}
			else
			{
				left = spaces + "    " + "null" + "\n";
			...
			if (n.right != null)
			{
				right = toStringRec(n.right, depth + 1);
			}
			else
			{
				right = spaces + "    " + "null" + "\n";
